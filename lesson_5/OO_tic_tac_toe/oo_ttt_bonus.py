...
class TTTGame:
    ...
    @staticmethod
    def _join_or(lst, delim=', ', word='or'):
        if len(lst) == 1:
            return lst[0]
        if len(lst) == 2:
            return f'{lst[0]} {word} {lst[1]}'

        lst = [str(choice) for choice in lst]

        return f'{delim.join(lst[:-1])} {word} {lst[-1]}'
    ...
    def match_over(self):
        return self.is_match_winner(self.human) or self.is_match_winner(self.computer)
    ...
    def display_match_score(self):
        ...
        print(f"Score: you - {human} computer - {computer}")
    ...
    def human_moves(self):
        ...
        while True:
            prompt = f"Choose a square ({TTTGame._join_or(valid_choices)}): "
            choice = input(prompt)
            try:
                choice = int(choice)
